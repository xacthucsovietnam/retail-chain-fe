...
export const getSupplierInvoiceDetail = async (id: string): Promise<SupplierInvoiceDetail> => {
    ...
    try {
        ...
        if (!response.data?.objects?.[0]) {
            throw new Error('Invalid cash supplier invoice detail response format');
        }
        ...
        return {
            ...
            author: supplierInvoice.author?.presentation || '',
            contract: supplierInvoice.contract?.presentation || '',
            counterparty: supplierInvoice.counterparty?.presentation || '',
            operationType: supplierInvoice.operationKind?.presentation || '',
            amount: supplierInvoice.documentAmount || 0,
            currency: supplierInvoice.documentCurrency?.presentation || '',
            employeeResponsible: supplierInvoice.employeeResponsible?.presentation || '',
            vatTaxation: supplierInvoice.vatTaxation?.presentation || '',
            structuralUnit: supplierInvoice.structuralUnit?.presentation || '',
            orderBasis: supplierInvoice.orderBasis?.presentation || '',
        ...
        console.error('Cash supplier invoice detail fetch error:', error);
        throw new Error('Failed to fetch cash supplier invoice details');
    }
};
